...
func TestKeysFromFolder(t *testing.T) {
	...
	vk, err := config.KeyListFromFolder("./keys_test_folder")
	...
	for i, key := range vk {
		...
		for _, expected := range expectedPublicKeys {
			if fmt.Sprintf("%x", key.ValidatorPubkey) == expected {
				found = true
				break
			}
		}
		if !found {
			t.Fatalf("expected pubkey %s, got %s", expectedPublicKeys[i], fmt.Sprintf("%x", key.ValidatorPubkey))
		}
	}
}
